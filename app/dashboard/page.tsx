  ...
  return (
    <div className="px-8 w-full min-h-screen flex flex-col gap-10 items-center pb-10 bg-gradient-to-br bg-background text-contrast">
      <div className="w-full pt-10">
        ...
        <p className="text-xl text-contrast">Welcome back! Here's your email overview.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
        {emailStats.map((stat, index) => (
          <div key={index} className="bg-secondary bg-opacity-50 backdrop-blur-lg rounded-2xl p-6 border border-gray-700 transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/20 hover:-translate-y-1">
      ...
      <div className="flex text-left w-full gap-10 h-[40vh] mt-20">
        <div className="flex flex-col h-full w-full items-start gap-4">
          ...
          <p className="text-xl text-contrast-secondary mb-4">Stay organized and boost your productivity with Mail.io</p>
          <button className="bg-purple-600 text-white px-8 py-3 text-xl rounded-xl hover:bg-purple-700 transition-colors">
            Compose New Email
          </button>
        </div>
            <div className="">
                <Image src="/logo.svg" className={`inset-0 ${(theme === "dark") ? "invert-0" : "invert"} -z-10 w-full h-full -translate-y-20`} alt="Sdf" width={100} height={100}/>
            </div>
      </div>
    </div>
  );
